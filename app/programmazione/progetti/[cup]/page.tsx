import { createProcedura } from "./actions";
...
export default function Page({ params }: Props) {
  ...
  async function onSubmit(formData: FormData) {
    "use server";
    return createProcedura(cup, formData);
  }
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Avvia Procedura — CUP: <span className="font-mono">{cup}</span></h1>
      <form action={onSubmit} className="rounded-xl border bg-white p-6 shadow-sm space-y-4 max-w-xl">
        <div>
          <label htmlFor="titolo" className="block text-sm font-medium text-slate-700">Titolo procedura</label>
          <input id="titolo" name="titolo" type="text" required placeholder="Es.: Lavori di riqualificazione..."
